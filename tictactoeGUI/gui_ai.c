...
static bool Btn(Rectangle r, const char* label, Color bg, Color fg) {
    Vector2 m = GetMousePosition(); bool hot = CheckCollisionPointRec(m, r);
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        // Buttons
        Rectangle bPVP={20,20,140,40}, bAI={170,20,160,40};
        Rectangle bE={340,20,50,40}, bM={395,20,70,40}, bH={470,20,50,40};

        if (Btn(bPVP, "Player vs Player", mode==0?GREEN:LIGHTGRAY, BLACK)) {
            mode=0; game_reset(&g); recorded=0; clickConsumed=true;
        }
        if (Btn(bAI, "Player vs AI", mode==1?GREEN:LIGHTGRAY, BLACK)) {
            mode=1; game_reset(&g); recorded=0; clickConsumed=true;
        }
        if (mode==1) {
            if (Btn(bE,"E",level==1?ORANGE:LIGHTGRAY,BLACK)) {level=1; clickConsumed=true;}
            if (Btn(bM,"M",level==2?ORANGE:LIGHTGRAY,BLACK)) {level=2; clickConsumed=true;}
            if (Btn(bH,"H",level==3?ORANGE:LIGHTGRAY,BLACK)) {level=3; clickConsumed=true;}
        }

        if (IsKeyPressed(KEY_R)) { game_reset(&g); recorded=0; }
        ...
        if (!clickConsumed && g.winner==0 && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            if(c>=0&&c<3&&r>=0&&r<3){
                ...
                if (mode==0 || (mode==1 && g.turn=='X')) {
                    if (game_make_move(&g, i)) game_check_end(&g);
                }
            }
        }

        // AI turn
        if (mode==1 && g.winner==0 && g.turn=='O') { game_ai_move(&g, level); game_check_end(&g); }
        ...
        if (mode==1) { DrawText("Difficulty:",270,70,20,DARKGRAY);
                       DrawText(level==1?"Easy":(level==2?"Medium":"Hard"),380,70,20,BLACK); }

        // Grid
        for(int i=1;i<3;i++){ DrawLine(OFFX,OFFY+i*CELL,OFFX+3*CELL,OFFY+i*CELL,BLACK);
        ...
        for(int i=0;i<9;i++){
            ...
            if (g.b[i]=='X'){ DrawLine(x-30,y-30,x+30,y+30,RED); DrawLine(x-30,y+30,x+30,y-30,RED); }
            else if (g.b[i]=='O'){ DrawCircleLines(x,y,38,BLUE); }
        }
        ...
        snprintf(line, sizeof(line),
            "%s totals  |  Games: %d   |  X wins: %d   |  O wins: %d   |  Draws: %d",
            (mode==0 ? "PvP" : "PvAI"), games, xw, ow, dr);

// Shift slightly higher, add wrapping space
