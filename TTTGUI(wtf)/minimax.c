...
static int minimax_cap(char b[9], int isMax, int depth, int maxDepth)
{
    ...
    if (maxDepth > 0 && depth >= maxDepth)  // reached depth cap
    {
...
int findBestMoveLvl(char b[9], int level)
{
    ...
    if (level == 1)
    {
        ...
        else  // 50% chance: use shallow search
        {
            best = -1000;       // start with low score
    ...
    else if (level == 2)  // Level 2: usually best, sometimes second, sometimes random
    {
        ...
        for (j = 0; j < n; j++)  // check all candidates
        {
            ...
            else if (value > second)  // maybe it is second-best
            {
        ...
        else if (i < 30)  // next 10% choose random
        {
            return empty[rand() % n];
        }
        else  // 70% choose best
    ...
    else  // Level 3: full minimax search (no depth cap)
    {
        best = -1000;       // start low
